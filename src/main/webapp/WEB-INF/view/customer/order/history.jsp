                        ...
                        <tbody>
                            <c:forEach var="order" items="${orders}">
                                <c:forEach var="orderDetail" items="${order.orderDetails}">
                                    <tr>
                                        <td><img src="/images/product/${orderDetail.product.image}" class="img-fluid" alt="${orderDetail.product.name}" style="width: 100px;"></td>
                                        <td>${orderDetail.product.name}</td>
                                        <td><fmt:formatDate value="${order.convertedOrderDate}" pattern="dd/MM/yyyy HH:mm:ss" /></td>
                                            ...
                                            <c:if test="${order.status == 'COMPLETED'}">
                                                ...
                                                <c:choose>
                                                    <c:when test="${orderDetail.productReview == null}">
                                                        <a href="/customer/product-review/${orderDetail.id}" class="btn btn-primary">Đánh giá sản phẩm</a>
